...
> **Status: Early prototype / actively evolving — v2.5.0**
...
> **自律ループは opt-in**: 無人で自走する「Mode D 自律ループ」(self-paced `/loop`)
> は既定 OFF(`[autonomy] enabled = false`)です。現状はプロンプト駆動で、Dock が
> [`mode-d-tick.md`](skills/garelier-dock/references/mode-d-tick.md) の手順
> (OBSERVE → GATE CHECK → DISPATCH → INTEGRATE → RECORD + 4 つの人間判断ゲート)に
> 従います。専用コマンド・cap 強制・doctor ガードはロードマップ段階です。
...
3. role worktree を外す: `git worktree remove <path>`(`__garelier/<pm_id>/_*/`)。
