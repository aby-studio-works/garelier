...
## 訂正 (W-045 実装時の再調査)
初動の root-cause note は `merge_land.test.sh:228` を主犯として挙げたが、実装時に
...
`merge-gate.sh` の `TARGET_ROOT_FOR_GIT` 解決) —
...
`merge_land.test.sh:228` はこの脆弱パターンの**見た目上の実例**として引用する
...
## 真因
merge-gate 系の **テスト fixture** (`merge_land.test.sh` / `dispatch_cleanup.test.sh`
等) が lock/request JSON を **シングルクォートの printf** で書き出しており、path
フィールドのシェル変数が展開されず literal 文字列で残る:

```bash
# merge_land.test.sh:228 付近 — printf format が ' で囲まれているため $DT が展開されない
...
**本番経路は無害**: `merge_request.sh:225` は `printf '"target_root": "%s"' "$(esc "$GIT_ROOT")"`
で正しく展開している。漏れるのは **テスト fixture の literal printf だけ**。

## 対策 (二層 — preventive + detective、W-045 実装済み)
1. **preventive (fixture 修正)**: `merge_land.test.sh` の `plant_lock.sh` ヒアドキュメント
...
2. **detective (恒久防御、本体)**: `target_root` を JSON から読んで spawn cwd / `cd` に使う
   ...
   **fallback にそのまま skip** する (旧実装は `resolve(fallback, target)` で無条件に
   ...
   - `skills/garelier-core/scripts/merge-gate.sh` の `TARGET_ROOT_FOR_GIT` 解決 (inline
     `bun -e` one-liner) — この値が最終的に `cd "$TARGET_ROOT"` される。
   `merge_request.sh` (書き込み側) と `dispatch_cleanup.sh` は `target_root` を mkdir/cd に
