# The jig — deterministic dispatch tick (DEC-062)
...
until then the prose Dock auto-loop tick (`role_subagent_dispatch.md` §4) is the
operative procedure and remains the fallback afterwards.

## What it is

The jig moves the dispatch tick's CONTROL FLOW out of model-interpreted prose
...
interactive PM only, in-session subagents / `codex exec` roles only, the
...
2. **PLAN** (one bounded model decision): choose which ready backlog items
   to dispatch this tick, within the current adaptive admission budget (advertised
   provider slots plus host CPU/memory/I/O capacity, charged by task resource class). Prefer items that retire
...
3. **DISPATCH** (code): roles run as worktree-isolated subagents on
...
4. **GATE** (code-enforced order): Guardian subagent, then Observer
   ...
   - NORMAL — plus one adversarial verifier prompted to REFUTE the
     role's report (kill on refute).
   - CRITICAL — `critical_roles` independent roles in parallel
...
5. **INTEGRATE + RECORD + CLEANUP** (zero-LLM, DEC-083): the mechanical tail
   ...
   next tick if a warm role exists); await timeout = ENQUEUED. Because
   ...
   (DEC-082 fix-2 — resume the role's OWN warm worktree with reviewer findings,
   up to `max_rework_rounds`, no cold re-implement) stays in the pipeline, since
   resuming the role needs the LLM.
...
- **Role death mid-task** (usage/quota limit, crash). A falsy role
  ...
  with a retry hint (warm-resume if the `_crew/dispatch<id>/checkout` still exists,
...
- **Warm rework, not cold re-implement.** On `NEEDS_REWORK`/`REFUTED`, the GATE
  stage resumes the role's OWN warm worktree (`produce({kind:'rework',
...
These keep the **role warm worktree as the unit of recovery**: a tick
...
Smith-window-only tick) before dispatching, so a `WORKING` `_crew/dispatch<N>/`
...
2026-07-03): a role backgrounded its gate against the foreground
...
A sub-agent (role, gate, preflight) is run-to-completion: a command it
...
dependency graph can exceed it — the trap that strands a role. **Prevent it,
...
- **Warm the cache from the MAIN session before dispatch.** The PM/operator runs
  ...
  background completion, so it is stall-immune — before launching a role
  dispatch on a cold cache (fresh clone, long idle, or a merge touching heavy
  shared components). With a warm build cache, every role gate build is
  incremental and fits the foreground limit. (Live: a cold role build detached
  + stalled at ~16 min; the same build warm finished in the foreground in ~9.5 min.)
- **Scope the role self-gate to the touched components** — the project's
  ...
  role commands under the foreground limit. (Concrete commands come from the
...
- **If a scoped gate still cannot fit the foreground limit on a warm cache, the
  role BLOCKs** (`gate exceeds foreground budget — needs a warm cache`); it
  does NOT detach-and-idle. The PM warms the cache from main and re-dispatches the
  role warm. A clean BLOCK is recoverable; a detach-and-idle is a silent stall.
- **Backstop (defense-in-depth, not the primary):** after dispatching a heavy
  role the operator may run `driver/src/scripts/dispatch_watch.ts --project <root>
  --pm-id <id> --id <N>` in the background (main session) — it polls the role's
  ...
  `stranded-role` (a WORKING dispatch with uncommitted work and no live
...
- `skills/garelier-core/templates/jig_tick.workflow.js` — the tick template
  ...
  dispatch runs (2026-06-11/12): roles start via `dispatch_prepare.ts`
  ...
  result = success AND the studio tip is gate-made), warning roles
  ...
  (an unfinished design never reaches a role), and a THIN context
  pack (no entry points / invariants / local-verify) dispatches with a
  warning telling the role to record what it had to rediscover under
  the report's "Context pack gaps" — `retro_digest` aggregates those at
  milestone close so recurring gaps improve the PM's blueprints; roles carry
...
- `skills/garelier-core/driver/src/scripts/jig_render.ts` — one-command render of the
  ...
  so the PM then calls
  `Workflow({ scriptPath, args: { admission: { provider_available_slots,
  host: { cpu_available_slots, memory_available_slots, io_available_slots } },
  items: [{ ..., resource_class: "light"|"normal"|"heavy" }] } })`. The caller
  supplies current advertised provider slots and current host headroom; it never
  invents availability or probes a provider. Partial telemetry is bounded by
  every available measurement, so a known zero remains zero. Only when no
  relevant provider or host measurement exists does the workflow log the
  condition and admit at most one non-heavy item as a bounded diagnostic; heavy
  and remaining items explicitly requeue until telemetry is wired. This
  compatibility fallback is not a static concurrency cap. CLI flags
  (`--smith-every`/`--depth-*`/`--out`) override config for a single run.
- `skills/garelier-core/templates/jig_gate_held.workflow.js` — the RESUME
  path: when a role finishes its work but returns BLOCKED (question /
  ...
  merge gate → record order WITHOUT re-running the role; pass
...
# No fan_out_cap: each tick uses current provider + host admission telemetry.
max_rework_rounds = 2    # bounded self-rework before PM escalation
critical_roles = 3   # N-version count for CRITICAL changes
...
critical = "nversion"    # N roles + judge panel + gate
```

## Per-seat model routing (Phase 3)

Each seat (role, refuter, judge, Guardian, Observer) is model-addressable.
Route by judgment density per `model_routing.md`: mid-tier on gated roles,
...
- **Routing on roles (W-025/W-026).** Each item's DISPATCH now runs a
  ...
  An explicit task flag is forwarded verbatim; agreement-range warnings are
  advisory and never alter the dispatch. The `produce:<slug>` label is taken from `dispatch_prepare`
...
- **Contract check (W-022).** When a role returns REPORTING, a mechanical
  ...
  violation with a warm role feeds the ready-made nudge back as a warm-rework
...
- If the script itself fails mid-tick, the Dock falls back to the
  prose Dock auto-loop tick for that cycle and reports the failure to PM.
