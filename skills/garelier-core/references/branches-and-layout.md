...
| `garelier/<target-slug>/<pm_id>/studio`                        | Dock / Artisan | Accepted role integration for the selected route |
...
The `satchel` family belongs to the Artisan Artisan route and `shelf`
to Dock orchestration. They may coexist; all writes to `studio` are serialized
by the merge-gate critical section.
...
│       ├── _crew/pm/                                      PM role's subdirectory (NOT a worktree)
│       ├── _crew/dock/                               Dock role's subdirectory (NOT a worktree)
│       ├── _crew/workers/<id>/                            Worker worktrees (one per Worker)
│       ├── _crew/scouts/<id>/                             Scout worktrees (one per Scout)
│       ├── _crew/smiths/<id>/                             Smith worktrees (one per Smith)
│       ├── _crew/librarians/<id>/                         Librarian worktrees (one per Librarian; Dock orchestration)
│       ├── _crew/observers/<id>/                          Observer worktrees (one per Observer; read-only sidecar, all applicable routes; detached HEAD)
│       ├── _crew/guardians/<id>/                          Guardian worktrees (one per Guardian; gavel branch, ephemeral, DEC-024)
│       ├── _crew/concierges/<id>/                         Concierge worktrees (one per Concierge; clipboard branch, local-only, DEC-025)
│       ├── _crew/artisan/                                 Artisan worktree (singleton; Artisan route)
│       ├── control/                                  This PM's persistent authority (tracked; schema selected by control.toml)
│       │   ├── control.toml                          Namespace/schema/mode identity
│       │   ├── project_dashboard/                    Current/Notes + curated/marker-bounded indexes
│       │   ├── roadmaps/                             Multiple canonical Roadmaps
│       │   ├── milestones/                           Shared/nested Milestone DAG nodes
│       │   ├── backlog/{open,archive/}               Canonical Backlogs
│       │   ├── backlog_views/                        Ordered queue/bundle definitions
│       │   ├── checkpoints/{active,archive/}         Durable resume records
│       │   ├── notes/                                Optional sharded durable Notes
│       │   ├── risks/{open,archive/}                 Risks
...
`__garelier/<pm_id>/_crew/pm/` and `__garelier/<pm_id>/_crew/dock/` are
...
`__garelier/<pm_id>/_crew/workers/<id>/`,
`__garelier/<pm_id>/_crew/scouts/<id>/`, and
`__garelier/<pm_id>/_crew/smiths/<id>/` ARE separate `git worktree`
...
Persistent role-container IDs identify stable local homes, not providers.
Prefer `<role>-NN`,
...
container. Task identity and provider/model/effort are selected per dispatch;
container metadata never supplies routing defaults. IDs only need to be unique
within one PM's tree — two PMs can each have their own `worker-01` without
collision.
...
| Typical writer    | Schema-3 control transactions; authorized artifact/report roles | Dock (manifest), Worker/Scout/Smith (inbox notifications) |
| Authority         | ControlModel source of truth for plans, decisions, rules | Source of truth only for "what is running right now" |

Always ask: does this file persist past the next driver poll? If yes,
it belongs in `control/`. If no, it belongs in `runtime/`.

Presence under `control/` does not authorize directory scans or direct edits:
use schema-3 bounded `session-open`/`get` reads and revision-checked CLI
transactions.
