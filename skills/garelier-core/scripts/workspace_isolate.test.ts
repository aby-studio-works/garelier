...
const shellScript = [
  "#!/usr/bin/env bash",
  "#",
  "# workspace_isolate.test.ts — pins workspace_isolate.ts's --collect",
  ...
  "#",
  "# Self-contained: run directly (`bun test workspace_isolate.test.ts`) or from",
  "# ci.ts. Exits 0 only if every branch holds.",
  "set -uo pipefail",
  "",
  "SELF_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"",
  "WI=\"$SELF_DIR/../driver/src/scripts/workspace_isolate.ts\"",
  "[ -f \"$WI\" ] || { echo \"workspace_isolate.test: cannot find workspace_isolate.ts next to me\" >&2; exit 1; }",
  "",
  "TMP=\"$(mktemp -d)\"",
  "trap 'rm -rf \"$TMP\"' EXIT",
  "",
  ...
  "  OUT1=\"$(bun \"$WI\" --repo \"$TMP\" --slug dirty-refuse)\" || fail \"isolate (dirty-refuse) failed\"",
  ...
  "",
  "  set +e",
  "  ERR1=\"$(bun \"$WI\" --collect --repo \"$TMP\" --slug dirty-refuse 2>&1 1>/dev/null)\"",
  "  RC1=$?",
  "  set -e",
  ...
  "  OUT2=\"$(bun \"$WI\" --collect --repo \"$TMP\" --slug dirty-refuse --force-collect)\" \\",
  ...
  "  OUT3=\"$(bun \"$WI\" --repo \"$TMP\" --slug clean-ok)\" || fail \"isolate (clean-ok) failed\"",
  "  WT3=\"$(echo \"$OUT3\" | sed -n 's/.*\"worktree\":\"\\([^\"]*\\)\".*/\\1/p')\"",
  "  echo \"clean work\" >> \"$WT3/file.txt\"",
  "  git -C \"$WT3\" add -A",
  "  git -C \"$WT3\" -c commit.gpgsign=false commit -qm \"clean work\"",
  "  OUT4=\"$(bun \"$WI\" --collect --repo \"$TMP\" --slug clean-ok)\" || fail \"clean collect (no force) failed\"",
...
test("workspace_isolate shell parity oracle", () => {
