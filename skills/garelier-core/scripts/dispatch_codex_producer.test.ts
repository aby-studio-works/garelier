...
const shellScript = [
  "#!/usr/bin/env bash",
  "#",
  "# dispatch_codex_producer.test.ts — pins the Codex subprocess sandbox contract:",
  "#",
  ...
  "#",
  "# Self-contained: run directly (`bun test dispatch_codex_producer.test.ts`) or from",
  "# ci.ts. Uses a fake `codex` on PATH; does not require the real Codex CLI.",
  "set -uo pipefail",
  "",
  "SELF_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd -P)\"",
  "PRODUCER=\"$SELF_DIR/../driver/src/scripts/dispatch_codex_producer.ts\"",
  ...
  "# source=\"\"), so the producer's assertRoutingMatches (the cf4df38+0a563a4",
  "# explicit-model contract) accepts the launch instead of a silent inheritance.",
  "write_context() {",
  "  printf '{\"project\":{\"project_root\":\"%s\",\"control_root\":\"%s\",\"target_root\":\"%s\"},\"routing\":{\"model\":\"%s\",\"effort\":\"%s\",\"source\":\"\"}}\\n' \\",
  "    \"$CONTEXT_TARGET\" \"$PROJECT\" \"$CONTEXT_TARGET\" \"$1\" \"$2\" > \"$CONTAINER/context.json\"",
  "}",
  "",
  "TMP=\"$(mktemp -d)\"",
  ...
  "    find \"$CODEX_EXPECT_SESSION_LOCK_DIR\" -name owner.json -type f -print -quit 2>/dev/null | grep -q . && break",
  "    sleep 0.05",
  "  done",
  "  find \"$CODEX_EXPECT_SESSION_LOCK_DIR\" -name owner.json -type f -print -quit 2>/dev/null | grep -q . || exit 93",
  "fi",
  "exit \"${CODEX_EXIT:-0}\"",
  "SH",
  ...
  "OUT=\"$TMP/out.txt\"",
  "ERR=\"$TMP/err.txt\"",
  "if ! CODEX_CONTEXT_FILE=\"$CONTAINER/context.json\" CODEX_EXPECT_SESSION_LOCK_DIR=\"$RESULT_DIR/locks\" CODEX_STREAM='{\"type\":\"thread.started\",\"thread_id\":\"thread-shell-1\"}' PATH=\"$FAKEBIN:$PATH\" bun \"$PRODUCER\" \\",
  "    --worktree \"$WORKTREE\" \\",
  "    --project \"$PROJECT\" \\",
  "    --prompt \"$PROMPT\" \\",
  "    --result \"$RESULT\" \\",
  ...
  "if ! PATH=\"$FAKEBIN:$PATH\" bun \"$PRODUCER\" \\",
  "    --worktree \"$WORKTREE\" --project \"$PROJECT\" --prompt \"$PROMPT\" --result \"$RESULT\" --add-dir \"$EXTRA\" \\",
  ...
  "",
  "# W-166: the remaining launches omit --model (model=\"\"), so reset the context",
  "# routing to empty to match — the assertRoutingMatches contract requires the",
  "# context routing to equal the launcher's (here both empty).",
  "write_context \"\" \"\"",
  ...
  "    PATH=\"$FAKEBIN:$PATH\" bun \"$PRODUCER\" \\",
  "    --worktree \"$WORKTREE\" --project \"$PROJECT\" --prompt \"$PROMPT\" --result \"$RESULT\" --add-dir \"$EXTRA\" \\",
  "    >\"$OUT\" 2>\"$ERR\"; then",
  ...
  "  \"$EXTRA/.agents\" \"$EXTRA/.codex\" \"$EXTRA/.git\" \"$PROJECT/.git\"",
  ...
  "rm -f \"$CODEX_ARGS_FILE\" \"$CODEX_STDIN_FILE\" \"$OUT\" \"$ERR\"",
  "if ! CODEX_HOME=\"$CODEX_HOME_EXISTS\" PATH=\"$FAKEBIN:$PATH\" bun \"$PRODUCER\" \\",
  "    --worktree \"$WORKTREE\" --project \"$PROJECT\" --prompt \"$PROMPT\" --result \"$RESULT\" \\",
  "    >\"$OUT\" 2>\"$ERR\"; then",
  ...
  "rm -f \"$CODEX_ARGS_FILE\" \"$CODEX_STDIN_FILE\" \"$OUT\" \"$ERR\"",
  "if ! CODEX_HOME=\"$CODEX_HOME_MISSING\" PATH=\"$FAKEBIN:$PATH\" bun \"$PRODUCER\" \\",
  "    --worktree \"$WORKTREE\" --project \"$PROJECT\" --prompt \"$PROMPT\" --result \"$RESULT\" \\",
  "    >\"$OUT\" 2>\"$ERR\"; then",
...
test("dispatch_codex_producer shell parity oracle", () => {
