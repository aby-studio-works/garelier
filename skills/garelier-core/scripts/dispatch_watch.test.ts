...
const shellScript = [
  "#!/usr/bin/env bash",
  "#",
  "# dispatch_watch.test.ts — pins dispatch_watch.ts --fleet mode branches (W-071).",
  "#",
  ...
  "#",
  "# Self-contained: run directly (`bun test dispatch_watch.test.ts`) or from ci.ts.",
  ...
  "  \"\") ;;",
  "  --fleet) FLEET_ONLY=1 ;;",
  "  *) echo \"usage: dispatch_watch.test.ts [--fleet]\" >&2; exit 2 ;;",
  "esac",
  "",
  "SELF_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"",
  "WATCH=\"$SELF_DIR/../driver/src/scripts/dispatch_watch.ts\"",
  "[ -f \"$WATCH\" ] || { echo \"dispatch_watch.test: cannot find dispatch_watch.ts next to me\" >&2; exit 1; }",
  "",
  "TMP=\"$(mktemp -d)\"",
  "trap 'rm -rf \"$TMP\"' EXIT",
  "",
  ...
  "run_fleet() {",
  "  set +e",
  "  OUT=\"$(bun \"$WATCH\" --fleet --project \"$TMP\" --pm-id \"$PM\" --interval-sec 1 \"$@\" 2>&1)\"",
  "  RC=$?",
  "  set -e",
  "}",
  "",
  ...
  "  set +e",
  "  bun \"$WATCH\" --project \"$TMP\" --pm-id \"$PM\" >/dev/null 2>\"$TMP/err5\"",
  "  RC5=$?",
  "  set -e",
  ...
  ") || exit 1",
  "",
  "# A focused, repeatable deflake target for CI/operator diagnosis. The full test",
  "# remains the default; --fleet intentionally stops before the unrelated",
  "# single-mode scenarios so `for i in {1..10}; do ... --fleet; done` measures the",
  "# temporal fleet assertion itself rather than aggregate suite duration.",
  "if [ \"$FLEET_ONLY\" -eq 1 ]; then",
  "  echo \"dispatch_watch.test: fleet-only branches pass (drain / gated-exclude / multi-watch / revive / usage)\"",
  "  exit 0",
  "fi",
  "",
  ...
  "run_single() {",
  "  set +e",
  "  OUT=\"$(bun \"$WATCH\" --project \"$REPO\" --pm-id \"$PM\" --branch wb \\",
  ...
  "  local id=\"$1\"; shift",
  "  set +e",
  "  OUT=\"$(bun \"$WATCH\" --project \"$REPO\" --pm-id \"$PM\" --id \"$id\" --branch wb \\",
  "    --interval-sec 1 --timeout-sec 1 \"$@\" 2>&1)\"",
  "  RC=$?",
  "  set -e",
  "}",
  ...
  "",
  "  # === D5 (W-143). Spawn/resume grace: a FRESH dispatched_at anchor SUPPRESSES",
  "  #     IDLE-DONE during the producer's premise-read/think phase (the #351/#352",
  "  #     false fire). Same active-then-static fixture as D1, but a just-written grace",
  "  #     marker -> no IDLE-DONE while inside the window, and the poll line marks",
  "  #     grace=1. (D1 proves the SAME fixture DOES fire without a marker, so D5's",
  "  #     non-fire + grace=1 isolates the suppression to the grace anchor.) =========",
  "  mkidle 34 WORKING no",
  "  date +%s > \"$SPR/_dispatch34/dispatched_at\"",
  "  ( i=0; while [ $i -lt 15 ]; do date >> \"$SPR/_dispatch34/checkout/f.txt\" 2>/dev/null; sleep 0.2; i=$((i+1)); done ) &",
  "  PERT34=$!",
  "  run_single_id 34 --windows 1 --timeout-sec 8 --proc-regex 'ZZZ_NO_MATCH'",
  "  kill \"$PERT34\" 2>/dev/null || true; wait \"$PERT34\" 2>/dev/null || true",
  "  [ \"$RC\" -eq 0 ] || fail \"D5 exit was $RC (expected 0): $OUT\"",
  "  echo \"$OUT\" | grep -q \"IDLE-DONE\" \\",
  "    && fail \"D5 a fresh spawn-grace anchor must SUPPRESS IDLE-DONE (read/think phase): $OUT\"",
  "  echo \"$OUT\" | grep -q \"grace=1\" \\",
  "    || fail \"D5 the poll line must mark grace=1 while inside the spawn grace: $OUT\"",
  "",
  "  # === D6 (W-143). A STALE dispatched_at anchor (epoch 100 = 1970, grace long",
  "  #     elapsed) must NOT suppress — a real stall still fires IDLE-DONE exactly as",
  "  #     D1. Grace only ever DELAYS, never removes, a genuine stall. ==============",
  "  mkidle 35 WORKING no",
  "  printf '100\\n' > \"$SPR/_dispatch35/dispatched_at\"",
  "  ( i=0; while [ $i -lt 15 ]; do date >> \"$SPR/_dispatch35/checkout/f.txt\" 2>/dev/null; sleep 0.2; i=$((i+1)); done ) &",
  "  PERT35=$!",
  "  run_single_id 35 --windows 1 --timeout-sec 8 --proc-regex 'ZZZ_NO_MATCH'",
  "  kill \"$PERT35\" 2>/dev/null || true; wait \"$PERT35\" 2>/dev/null || true",
  "  [ \"$RC\" -eq 0 ] || fail \"D6 exit was $RC (expected 0): $OUT\"",
  "  echo \"$OUT\" | grep -q \"grace=1\" \\",
  "    && fail \"D6 an elapsed grace anchor must not mark grace=1: $OUT\"",
  "  echo \"$OUT\" | grep -q \"^RESULT: IDLE-DONE\" \\",
  "    || fail \"D6 a stale grace anchor must still let a real stall fire IDLE-DONE: $OUT\"",
  ") || exit 1",
  "",
...
test("dispatch_watch shell parity oracle", () => {
  const fleetOnly = process.argv.includes("--fleet") || process.env.GARELIER_DISPATCH_WATCH_FLEET_ONLY === "1";
  const result = runShellOracle(shellScript, import.meta.path, fleetOnly ? ["--fleet"] : []);
