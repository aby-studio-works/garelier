...
const shellScript = [
  "#!/usr/bin/env bash",
  "#",
  "# gate_result_waiter.test.ts — pins gate_result_waiter.ts's branches (W-079).",
  "#",
  ...
  "#",
  "# Self-contained: run directly (`bun test gate_result_waiter.test.ts`) or from ci.ts.",
  "# Exits 0 only if every branch holds.",
  "set -uo pipefail",
  "",
  "SELF_DIR=\"$(cd \"$(dirname \"$0\")\" && pwd)\"",
  "WAITER=\"$SELF_DIR/../driver/src/scripts/gate_result_waiter.ts\"",
  "[ -f \"$WAITER\" ] || { echo \"gate_result_waiter.test: cannot find gate_result_waiter.ts next to me\" >&2; exit 1; }",
  "",
  "TMP=\"$(mktemp -d)\"",
  "trap 'rm -rf \"$TMP\"' EXIT",
  "",
  ...
  "mkdir -p \"$RES\"",
  "",
  "# Write a realistic result file (subset of merge-gate.ts write_result output) for",
  ...
  "  set +e",
  "  OUT=\"$(bun \"$WAITER\" --project \"$TMP\" --pm-id \"$PM\" --request-id \"$1\" \"${@:2}\")\"",
  "  RC=$?",
  "  set -e",
  "}",
  "",
  ...
  "  # === 6. bad args (missing --request-id) -> exit 2 ==========================",
  "  set +e",
  "  bun \"$WAITER\" --project \"$TMP\" --pm-id \"$PM\" >/dev/null 2>\"$TMP/err6\"",
  "  RC6=$?",
  "  set -e",
...
test("gate_result_waiter shell parity oracle", () => {
