#!/usr/bin/env bash
# Tests for merge_request_id_recover.sh (W-064).
set -euo pipefail
SELF_DIR="$(cd "$(dirname "$0")" && pwd -P)"
SUT="$SELF_DIR/merge_request_id_recover.sh"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
fail=0
ok()   { echo "  ok: $1"; }
...
got="$(bash "$SUT" --stderr-file "$ERR1" --requests-dir "$REQ_DIR" --since 0)"
...
got="$(bash "$SUT" --stderr-file "$ERR2" --requests-dir "$REQ_DIR" --since 0)"
[ "$got" = "$RID" ] && ok "newest-file recovery" || bad "newest-file recovery: got '$got'"

# 3. stale guard: since far in the future → no recovery, exit 1
if bash "$SUT" --stderr-file "$ERR2" --requests-dir "$REQ_DIR" --since 99999999999 >/dev/null 2>&1; then
...
got="$(bash "$SUT" --stderr-file "$ERR4" --requests-dir "$REQ_DIR" --since 0)"
...
got="$(bash "$SUT" --stderr-file "$ERR5" --requests-dir "$REQ_DIR" --since 0)"
[ "$got" = "$RID" ] && ok "relative-path basename retry" || bad "relative-path basename retry: got '$got'"

[ "$fail" -eq 0 ] && echo "merge_request_id_recover.test.sh: ALL PASS" || { echo "merge_request_id_recover.test.sh: FAILURES" >&2; exit 1; }
