...
// W-054: dispatch_prepare.sh must accept a glob-shaped --touches value (e.g.
...
const DP = join(import.meta.dir, "..", "..", "scripts", "dispatch_prepare.sh");
...
describe("dispatch_prepare.sh --touches glob quoting (W-054)", () => {
  test("a SINGLE-quoted glob value survives the shell and is accepted (not mis-parsed)", () => {
    ...
    const cmd = `bash '${DP}' --project '${dir}' --pm-id p --role worker --slug s --touches 'docs/**'`;
