...
# \`track-target.md\` trigger the role consumes at its next iteration boundary
...
# BY HAND for every role — a mechanism with no reachability (DEC-067 class).
...
# conflicts — the ROLE performs the merge and owns any conflict (DEC-039 does
# not widen Dock's no-code-writing boundary). This tool only measures + (with
# --write) drops the trigger file into the role's own container.
#
# Eligibility (mirrors §8.5/§8.6): only a role whose STATE.md Status is
# WORKING and whose checkout is on a \`.../workbench/...\` or \`.../anvil/...\` branch
# is a candidate — a BLOCKED/REPORTING/REVIEWING/REWORK role must not be
# instructed, and scout/observer/guardian/concierge have no
# forward-integrated branch. Containers scanned: the dispatch-native
# \`_crew/dispatch<N>/\` homes plus in-project persistent \`_crew/workers/<id>/\` and
# \`_crew/smiths/<id>/\` containers (DEC-036 default). Exile-relocated containers
...
#   --write         actually drop track-target.md for each eligible role
...
#    "roles":[{"container","role","branch","behind","pending","action","wrote"}]}
...
export async function main(argv = process.argv.slice(2)): Promise<number> {
  ...
  if (!studio) studio = readTomlQuoted(`${base}/_crew/pm/setup_config.toml`, "integration");
  ...
  const scan = (container: string): void => {
    ...
    const row: Role = { container: relative, role, branch, behind: 0, pending, action: "current", wrote: false };
    if (!studioExists) {
  ...
  for (const path of childDirs(`${base}/_crew`).filter((x) => /[\\/]dispatch[^\\/]*$/.test(x))) scan(path);
  for (const path of childDirs(`${base}/_crew/workers`)) scan(path);
  for (const path of childDirs(`${base}/_crew/smiths`)) scan(path);

  if (format === "text") {
    process.stdout.write(`base-tracking scan: pm=${pm} studio=${studio || "<unresolved>"} threshold=${threshold} mode=${mode}\n`);
    process.stdout.write(`  scanned=${roles.length} triggered=${triggered}\n`);
    // Preserve the shell implementation's rendering: its comma split means no
    // role detail line matches the later three-field sed expression.
  } else {
    emitJsonLine({
      pm_id: pm,
      studio,
