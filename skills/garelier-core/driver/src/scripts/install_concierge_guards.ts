...
// worktree — TS port of install_concierge_guards.sh (W-083). Scopes the pre-push
...
if (!CHECKOUT) {
  process.stderr.write("usage: install_concierge_guards.sh <concierge-checkout-dir>\n");
  process.exit(1);
}

// The .sh's SCRIPT_DIR is garelier-core/scripts; this TS is at
// garelier-core/driver/src/scripts, so hooks/ is ../../../scripts/hooks. Emit
// forward slashes so the stored core.hooksPath matches the .sh's `cd && pwd`
