...
// dispatch_cleanup.sh's merge_status_for_branch): is the workbench branch now an
// ancestor of HEAD? If so → success (report the landed commit); else → aborted;
// an unresolved branch fails closed to aborted.
//
// This is the SAME decision the merge-gate.sh parity oracle (the retained bash
// cleanup_and_abort(), pinned by merge_gate_landed_check.test.sh) encodes, kept
...
// lines (status, commit) — the drive point for merge_gate_landed_check.test.sh.
if (import.meta.main) {
