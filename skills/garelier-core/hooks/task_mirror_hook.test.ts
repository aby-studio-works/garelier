...
const shellScript = [
  ...
  "EOF",
  "",
  "# W-141: task_mirror --format ops mirrors only the ACTIVE BAND (in-flight",
  "# dispatches + the ids the PM names in current.md), so a fixture with no active",
  "# tasks yields an EMPTY mirror and the hook writes no baseline. Name the fixture",
  "# ids in current.md — the realistic 'these are active right now' signal — so the",
  "# mirror includes them (this pinned the empty-band regression on BOTH platforms;",
  "# the file lives outside driver/src so `bun test src/` never exercised it).",
  "cat > \"$BL/current.md\" <<'EOF'",
  "# Current focus",
  "",
  "## Execution queue",
  "- W-001 First task",
  "- W-002 Second task",
  "- W-003 Third task (added mid-test)",
  "EOF",
  "",
