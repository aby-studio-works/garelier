...
- **あなた / user** が方向(依頼、または roadmap + backlog)を決め、2 つの
  ...
  承認です。
- **PM** は方向を *blueprint*(小さな仕様)に変え、execution route を選び、promote を承認
  します。PM はソースを編集しません。
- **Roles** が手を動かします: *Worker*(実装)、*Smith*(merge 後の
...
- **Dock** は統合ブランチ(`studio`)を持ち、role を dispatch し、受理した
...
3. **実装。** Dock が role(または Artisan)を dispatch します。成果は専用
...
- role が REPORTING なのに report が無い、lock が stale に見える → **Guide →
  Diagnostics** に確認順(execution route → merge gate → role STATE)があります。
