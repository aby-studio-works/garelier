...
<#
.SYNOPSIS
    Garelier Setup Wizard (PowerShell version) — v2.6.3.
...
'@ | Set-Content -Path $libReadme -Encoding utf8
    ...
    [void]$sb.AppendLine('garelier_version = "2.6.3"')
    [void]$sb.AppendLine('')
...
'@
    ...
    [void]$mb.AppendLine('Garelier version: 2.6.3')
    ...
    Write-Host "       git commit -m 'Garelier: initialize PM $script:PmId (v2.6.3)'"
    ...
    foreach ($line in $tomlLines) {
        ...
        if ($rewrite -match '^garelier_version\s*=\s*"2\.([015]\.0|6\.[012])"') {
            $rewrite = 'garelier_version = "2.6.3"'
        }
        if ($rewrite -match '^wizard_version\s*=\s*"2\.([015]\.0|6\.[012])"') {
            $rewrite = 'wizard_version = "2.6.3"'
        }
        $output.Add($rewrite)
    }
