...
'@ | Set-Content -Path $libReadme -Encoding utf8
    ...
    Write-Host ''
    Write-Host "==> Generating $pmRoot/_pm/.claude/settings.json (SessionStart digest + SessionEnd hook)..."
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "test -n \"${GARELIER_DRIVER:-}\" || { mkdir -p ../runtime/driver && touch ../runtime/driver/stop; }"
          }
        ]
      }
    ]
