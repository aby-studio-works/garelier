...
<#
.SYNOPSIS
    Garelier Setup Wizard (PowerShell version) — v2.8.3.
...
'@ | Set-Content -Path $libReadme -Encoding utf8
    ...
    [void]$sb.AppendLine('garelier_version = "2.8.3"')
    [void]$sb.AppendLine('')
...
'@
    ...
    [void]$mb.AppendLine('Garelier version: 2.8.3')
    ...
    Write-Host "       git commit -m 'Garelier: initialize PM $script:PmId (v2.8.3)'"
    ...
    foreach ($line in $tomlLines) {
        ...
        if ($rewrite -match '^garelier_version\s*=\s*"\d[\d.]*"') {
            $rewrite = 'garelier_version = "2.8.3"'
        }
        if ($rewrite -match '^wizard_version\s*=\s*"\d[\d.]*"') {
            $rewrite = 'wizard_version = "2.8.3"'
        }
        $output.Add($rewrite)
    }
