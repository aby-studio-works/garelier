...
<#
.SYNOPSIS
    Garelier Setup Wizard (PowerShell version) — v2.6.2.
...
'@ | Set-Content -Path $libReadme -Encoding utf8
    ...
    [void]$sb.AppendLine('garelier_version = "2.6.2"')
    [void]$sb.AppendLine('')
...
'@
    ...
    [void]$mb.AppendLine('Garelier version: 2.6.2')
    ...
    Write-Host "       git commit -m 'Garelier: initialize PM $script:PmId (v2.6.2)'"
    ...
    foreach ($line in $tomlLines) {
        ...
        if ($rewrite -match '^garelier_version\s*=\s*"2\.([015]\.0|6\.[01])"') {
            $rewrite = 'garelier_version = "2.6.2"'
        }
        if ($rewrite -match '^wizard_version\s*=\s*"2\.([015]\.0|6\.[01])"') {
            $rewrite = 'wizard_version = "2.6.2"'
        }
        $output.Add($rewrite)
    }
