...
<#
.SYNOPSIS
    Garelier Setup Wizard (PowerShell version) — v2.7.0.
...
'@ | Set-Content -Path $libReadme -Encoding utf8
    ...
    [void]$sb.AppendLine('garelier_version = "2.7.0"')
    [void]$sb.AppendLine('')
...
'@
    ...
    [void]$mb.AppendLine('Garelier version: 2.7.0')
    ...
    Write-Host "       git commit -m 'Garelier: initialize PM $script:PmId (v2.7.0)'"
    ...
    foreach ($line in $tomlLines) {
        ...
        if ($rewrite -match '^garelier_version\s*=\s*"2\.([015]\.0|6\.[012345])"') {
            $rewrite = 'garelier_version = "2.7.0"'
        }
        if ($rewrite -match '^wizard_version\s*=\s*"2\.([015]\.0|6\.[012345])"') {
            $rewrite = 'wizard_version = "2.7.0"'
        }
        $output.Add($rewrite)
    }
