#!/usr/bin/env bash
#
# Garelier Setup Wizard (bash) — v2.8.1
#
# Three modes:
...
rewrite_setup_config_version() {
    ...
    [ -f "$toml" ] || return 0
    sed -i.bak \
        -e "s|^garelier_version = \"[0-9][0-9.]*\"|garelier_version = \"2.8.1\"|" \
        -e "s|^wizard_version = \"[0-9][0-9.]*\"|wizard_version = \"2.8.1\"|" \
        "$toml"
    rm -f "$toml.bak"
}
...
EOF
    ...
    echo "       git commit -m 'Garelier: initialize PM $PM_ID (v2.8.1)'"
