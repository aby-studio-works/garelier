#!/usr/bin/env bash
#
# Garelier Setup Wizard (bash) — v2.11.3
#
# Three modes:
...
rewrite_setup_config_version() {
    local toml="$1"
    [ -f "$toml" ] || return 0
    sed -i.bak \
        -e "s|^garelier_version = \"[0-9][0-9.]*\"|garelier_version = \"2.11.3\"|" \
        -e "s|^wizard_version = \"[0-9][0-9.]*\"|wizard_version = \"2.11.3\"|" \
        "$toml"
    rm -f "$toml.bak"
}
...
EOF
    ...
    echo "==> Generating $PM_ROOT/_pm/setup_config.toml..."
    {
        ...
        echo "# runtime/merge_gate/archive/ is retained by [merge_gate]"
        echo "# archive_keep_days (default 14, W-038) instead of this block —"
        echo "# see garelier-core/retention.md."
        echo ""
    ...
    if [ "$GIT_ROOT" = "$PROJECT_ROOT" ]; then
        ...
        echo "       git commit -m 'Garelier: initialize PM $PM_ID (v2.11.3)'"
    else
