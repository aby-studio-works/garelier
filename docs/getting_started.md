...
> Status: draft (v2.5.0). The setup wizard described here is implemented
> in `skills/garelier-pm/scripts/setup_wizard.{sh,ps1}`.
...
promote します。無人で自走させたい場合のみ、opt-in の Mode-D `/loop`
(既定 OFF)を arm します。

> **DEC-061 — dispatch-only.** headless `claude -p` ドライバ(Mode B)は無効化され、
> `start_driver` も driver エントリも起動を拒否します。provider 多様化(Codex)は
> dispatch producer として維持されます。詳細・歴史的経緯は
> [execution_backends.md](execution_backends.md) を参照。
...
- Bun は driver / merge gate / status web console に必要です。未導入の場合、
  setup wizard が初回プロジェクト初期化時に確認して best-effort install /
  driver dependency setup / Mermaid vendoring を実行できます。
- Guardian role を有効化する場合、`[guardian_tools]` が指定する secret/PII
  scanner(既定は gitleaks)を PATH に通すこと。setup wizard は Guardian gates が
  設定され、gitleaks が未導入のときだけ確認します。Claude Code の driver /
  autonomous mode では既定 gitleaks の Guardian 許可は driver が付与するが、
  custom scanner は project-local allowlist が必要な場合がある。
...
キャッシュ配置のため、手動の copy / symlink は不要です。`<owner>/<repo>` は
公開先の GitHub リポジトリに読み替えてください。導入後は
...
```bash
export CLAUDE_PLUGIN_ROOT=<path to your garelier checkout>
```

```powershell
$env:CLAUDE_PLUGIN_ROOT = "<path to your garelier checkout>"
```
...
- `__garelier/<pm_id>/_workers/<id>/`, `_scouts/<id>/`, `_smiths/<id>/` と、
  `_librarians/<id>/` / `_observers/<id>/` / `_guardians/<id>/` /
  `_concierges/<id>/`、指定があれば `_artisan/` の worktree 作成
- 各 worktree に最小 CLAUDE.md を配置
...
- `AGENTS.md` をテンプレートから生成(言語・build/test・quality gate は
  `--stack` と quality gate から自動補完。restricted files §3 と conventions §5
  だけ `{{placeholder}}` が残る)
- `__garelier/<pm_id>/control/operations/promote_checklist.md` 等の安全ルールを
  シードとして配置

> **重要(初回の必須手順):** fresh setup 直後の `AGENTS.md` には、プロジェクト
> 固有の restricted files / conventions が `{{...}}` のまま残ります。`doctor` は
> これを **P0 placeholder-leak** として検出し、driver は起動を拒否します
> (これは安全側の仕様)。launch 前に `AGENTS.md` を一度編集して `{{...}}` を
> 埋めてください。`setup_wizard` を再実行しても既存 `AGENTS.md` は上書きされない
> ため、placeholder は埋まりません。
...
PM は `setup_config.toml` の変更を検出し、新しい worktree のみを生成、
既存のものには触りません。**追加前に `<target>` を
...
- **doctor**(read-only 健康診断): 1 PM のインストールを点検し、
  ...
  `lane.lock` / lease、version drift 等を検出します。`start_driver` は
  pre-flight で doctor を実行し、P0 があると `--force` / `-Force` 無しでは
  起動しません。
...
- **status**(CLI 一覧): driver / lane / 各ロール(Worker / Scout / Smith /
  Artisan / Librarian / Observer / Guardian / Concierge)の状態、gate request/result、
  backlog、最近のアクティビティを表示します。`--watch <秒>` で定期更新。

- **status web console**(ローカル read-only ブラウザビュー): lane / driver /
  merge gate / ロール表 / 最近のレポート等を loopback 限定で表示します。AI
  トークンを消費せず、状態を変更しません。
...
- **session digest**(対話 PM 起動時の自動サマリ): hybrid / manual モードで
  `__garelier/<pm_id>/_pm/` を直接開く運用を選んだ場合、SessionStart フックが
  lane / driver / inbox 件数 / merge-gate・observer results / stale leases /
  doctor サマリを数行で提示します。**AI を呼ばない決定論的出力**なので
...
1. **実行を停止する。** dispatch の `/loop` を arm 済みなら止めます(PM に「止めて」
   と依頼)。
2. **全 role を `IDLE` にする。** 作業中の role があれば完了を待つか、PM の
   clean stop 手順(`abort.md`)で停止します。`status` で各 role 状態を確認できます。
3. **role worktree を外す。** `__garelier/<pm_id>/_*/` の各 worktree を
...
- **fresh setup 直後に `doctor` が P0 / driver が起動しない (placeholder-leak)**
  `AGENTS.md` のプロジェクト固有フィールド(restricted files §3、conventions §5)が
  `{{...}}` のまま残っています。これは仕様です — 自律エージェントが従うべき
  プロジェクト規約が未定義のまま起動しないようにブロックしています。`AGENTS.md`
  を編集して `{{...}}` を埋めてください。言語・build/test・quality gate は
  `--stack` から自動補完済みです。`setup_wizard` の再実行は既存 `AGENTS.md` を
  スキップするため placeholder は埋まりません。どうしても先に起動したい場合のみ
  `garelier driver --force`(PowerShell は `garelier driver -Force`)。
