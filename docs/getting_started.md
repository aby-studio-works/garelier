...
> Status: draft (v2.6.0). The setup wizard described here is implemented
...
> これを **P0 placeholder-leak** として検出します — P0 が残ったまま dispatch
> ループを arm しないでください(これは安全側の仕様)。launch 前に `AGENTS.md` を
> 一度編集して `{{...}}` を埋めてください。`setup_wizard` を再実行しても既存
> `AGENTS.md` は上書きされないため、placeholder は埋まりません。
