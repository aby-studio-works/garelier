...
flowchart TD
  ...
  PM -.opt-in 設計レビュー · DEC-076.-> WD[Wanderer\n外部アドバイザリ peer]
...
| **Wanderer** | 助言 + sign-off | (なし — 外部) | アドバイザリ・レビュー役 (DEC-076): 確定前の PM 設計を peer-channel 越しにレビューする外部の opt-in peer。commit 無・決定なし。不在時は Observer。上の *PM design review* を参照。 |
