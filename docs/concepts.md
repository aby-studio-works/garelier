...
> Status: draft (v2.5.0). This file is the canonical human-readable
> reference for the Garelier design rationale.
...
- **永続正本と実行状態の分離** — 各 PM の
  ...
  driver pids)。両者は git 管理粒度もライフタイムも別。
...
- **運用者の確認事項**: 利用前に、自分のプラン・地域・プロバイダの最新 ToS で、
  自動化・subagent・無人実行の扱いを各自で確認してください。疑義があれば
  プロバイダに確認するのが安全です。
...
| 場所 | git 状態 | 主な利用者 |
| ---- | -------- | ---------- |
| primary checkout | `garelier/<target-slug>/<pm_id>/studio` | PM / Dock |
| `__garelier/<pm_id>/_pm/` | primary checkout 内の通常ディレクトリ | PM |
| `__garelier/<pm_id>/_dock/` | primary checkout 内の通常ディレクトリ | Dock |
| `__garelier/<pm_id>/_workers/<id>/` | Worker ごとの container。git worktree は `checkout/` に nest(DEC-020)、作業中は `workbench` branch | Worker |
| `__garelier/<pm_id>/_scouts/<id>/` | Scout ごとの worktree。常に detached HEAD | Scout |
| `__garelier/<pm_id>/_smiths/<id>/` | Smith ごとの worktree。作業中は `anvil` branch | Smith |
...
**Garelier の実行は dispatch のみ(DEC-061)** です — PM 対話セッション内の
サブエージェント(および Codex 割当ロールは `codex exec` subprocess)として
ロールを実行します。**headless driver(Mode B, `claude -p`)は無効化**され、
`start_driver` も driver エントリも起動を拒否します。`[execution] backend`
(`headless`/`codex`)は無効化された driver 専用の歴史的な軸で、dispatch には
用いません。provider 多様化(Codex)は dispatch producer として維持されます。
...
`[execution] backend`(無効化された headless driver 用の歴史的な選択肢):
- `headless`: 従来の `claude -p`。`[execution]` セクションが無い旧 config も
  後方互換でこの値が既定になりますが、DEC-061 によりこの値に関わらず driver は
  起動しません。
- `codex`: イテレーションを Codex CLI(`codex exec`)で実行。ロール単位の
  `provider = "codex-cli"` 明示指定も尊重されます。
