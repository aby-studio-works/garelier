...
| S-18 | Artisan takes one task end-to-end on `satchel` and integrates it into `studio` with mandatory Guardian + Observer review (DEC-045) | PM, Artisan, Guardian, Observer | Driver killed after quality/coverage audits pass but before studio integration | On restart Artisan resumes from checkpoint, forward-integrates studio, obtains Guardian then Observer verdicts pinned to studio/satchel SHAs, then submits through merge gate only while expected SHA is current | Pass |
...
| G-14 | Guardian report contains an unredacted secret-like value | doctor P0 `guardian-report-leak` (output safety): scans `_crew/guardians/*/guardian_report.md` + `runtime/guardian/{results,inbox}/*` for high-confidence secret formats (private keys, cloud/provider tokens, JWTs); redaction placeholders never match (doctor.ts, DEC-024) |
...
| C-32 | An Artisan crashes with a pending merge request | restart preserves the request/result; stale expected SHA triggers forward-integration and the required re-gate cycle before a new request |
...
- Artisan task interrupted mid-flight; the merge request/result
  route parked until the Artisan resumes or PM resolves it.
