...
│   ├── _pm/                              PM ロール
│   ├── _dock/                       Dock ロール
│   ├── _workers/<worker_id>/             Worker container: 調整ファイル + checkout/ worktree、in-project(DEC-036; exile は opt-in)
...
│       ├── manifest.md                   この PM の live agent index
│       ├── backlog/
│       │   ├── pending.md                未着手・進行中
│       │   ├── in_flight.md
...
│       └── driver/                       autonomous mode の pids / stop / logs
└── <pm_id-B>/                            ← 別 PM、完全に独立
    └── ... (同じ shape)
```

`runtime/driver/pids/` は detached Worker / Scout / Smith / Artisan /
Librarian / Observer / Guardian / Concierge の lease を置く場所です。ファイル名は `<role>-<id>.pid`
(`worker-worker-02.pid` など)。新しい lease は `.pid` suffix ですが
中身は JSON で、`pid`, `assignment_hash`, `branch`, `started_at`,
`status` を持ちます。driver は live lease がある agent を二重起動せず、
finished lease は次 poll で消費し、dead stale lease は削除してその role の
mtime snapshot を invalidation します。
...
(identity は driver の `--append-system-prompt-file` で正典＝prompt-
authoritative)。wizard はプロジェクト内のまま `<checkout>/.claude/
settings.local.json` に `claudeMdExcludes`(本流 `CLAUDE.md` 群の絶対 glob)を
...
`<role 単数>.<id>=<絶対 container>`、加えて `artisan=…`)に記録されます。driver
...
`CLAUDE.md` の絶対パス(driver が `--append-system-prompt-file` で再注入)で
...
**`__garelier/<pm_id>/control/` は存在しません。** 2 PM 間で tracked
...
- `runtime/backlog/in_flight.md` の task 行を、同じ task id のまま
  `runtime/backlog/pending.md` に戻します。
...
| `__garelier/<pm_id>/runtime/backlog/in_flight.md`                    | Dock            | 全員(同 PM)  |
...
| `__garelier/<pm_id>/runtime/merge_gate/locks/` (`active.lock` 含む)  | merge-gate subprocess / driver | driver    |
...
| `__garelier/<pm_id>/_<role>/<id>/urgent.md` (任意の detached agent)   | PM or Dock      | driver (DEC-031) |
...
| `__garelier/<pm_id>/runtime/lane.lock`                               | lane holder (Artisan/Dock) | driver, 全員 |
...
| `__garelier/<pm_id>/runtime/observer/{requests,results}/`            | Requester (requests) / Observer (results) | Observer, driver |
...
| `__garelier/<pm_id>/runtime/guardian/{requests,results}/`            | Requester (requests) / Guardian (results) | Guardian, driver |
...
| `__garelier/<pm_id>/runtime/concierge/{requests,results,locks}/`     | PM (requests) / Concierge (results, target-scoped locks) | Concierge, driver |
