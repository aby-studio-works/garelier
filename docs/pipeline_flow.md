...
the command chain, the execution routes, the roles, the branches,
...
flowchart TD
  ...
  PM[PM\ndecide · approve · pick route · promote] -->|blueprint + route| ROUTE{execution route?}
...
PM selects an execution route per task; no route is fixed or default. Role
routes may run concurrently. At most one integrator writes `studio` at a time:
every integration enters the shared merge-gate critical section
`runtime/merge_gate/locks/active.lock`.

- **Dock orchestration** — the coordinated path. PM authors a blueprint and
  hands it to **Dock**, which owns `studio`, dispatches work, reviews
  reports, and sends accepted role branches through the **merge gate**.
  ...
  target; they never merge.
- **Artisan single-role** — one **Artisan** performs the combined
  Dock+Worker+Scout+Smith+Librarian scope on a `satchel` branch and submits it
  through the merge gate after its own quality gate and required Guardian -> Observer
  checks. PM then approves any promote and Concierge executes it. The requester
  for role gates is the Artisan, not Dock. Its ceremony — singleton, satchel
  branch, Guardian -> Observer, merge request — is the single-role
  integration contract, not a tax on every small subagent launch. If the
  expected studio SHA is stale, it forward-integrates and repeats the gates.
- **PM-directed lightweight** (DEC-093) — a lightweight route for control / docs /
  ...
  At most one role writes the integration branch at once (parallel work goes
  on isolate branches). When unsure whether the criteria hold, fall to the
  Dock orchestration — this route is never a way to skip a gate.
...
early, before any role builds against it. The primary reviewer is the
...
with a verdict and advice. It takes no execution route or branch, makes no commits, and
...
| --- | --- | --- | --- |
| **PM** | decisions | (none) | Never edits source; selects an execution route; approves promote/external ops. |
...
| **Artisan** | commits | `satchel/#id` | Artisan route; integrates into `studio` after its gates. |
...
When Dock integrates a role branch, the mechanical part (base-track,
merge, run configured quality gates) runs as an **async subprocess**. Dock
dispatches the request and later verifies the result, so other roles can
...
integrating into `studio`. Both execution routes use the same PM approval + Concierge
...
`track-target.md` trigger; the role merges `studio` at its next iteration
